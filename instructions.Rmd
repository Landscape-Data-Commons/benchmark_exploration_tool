...
### 4) Decide your indicator label
By default, the indicator label will be the same as the variable name. It may make sense to change the label to be clearer or more human-friendly, e.g. changing "Foliar.Cover.Pct" to "Foliar cover (%)".

### 5) Decide between generating box plots and histograms
If you select box plots, the figures produced will be box plots of the indicator value's distribution broken into quartiles (at 25%, 50%, and 75%) with scatterplots representing the data points.

If you select histograms, the figures produced will be histograms showing the indicator value's distribution, broken into quantiles.

The base histogram will be broken into quantiles according to the user-defined percentages. As an example, entering "33, 67" will result in showing the value for the variable below which 33% percent of observations fall and the value of the variable below which 67% of the observations fall. The percentages entered must be numbers between 0 and 100 separated by commas. If your entry is invalid, the figure will default to showing the 50% break.
...
If you use your current data, you must specify the variable in the data which is unique for each observation (e.g., a plot key) and then you may choose one or more of the IDs in that variable to draw your values from. The values used will be for the indicator you are currently plotting.
